...
if __name__ == '__main__':
    try:
        ...
        print('\n', 'The biggest bar'.center(36, '*'), '\n\n', json.dumps(get_biggest_bar(python_obj_data), ensure_ascii=False, indent=4))
        print('\n' ,'The smallest bar'.center(36, '*'), '\n\n', json.dumps(get_smallest_bar(python_obj_data), ensure_ascii=False, indent=4))
        print('\n' ,'The closest bar'.center(36, '*'), '\n\n', json.dumps(get_closest_bar(python_obj_data, argv[2], argv[3]), ensure_ascii=False, indent=4))
    except ValueError:
